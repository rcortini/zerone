...
axis(side=2, at=c(0,40,80,120), cex.axis=0.8)
title(ylab="Read count", line=2)
text(x=-7000, y=115, "JARID1A (rep1)", pos=4, cex=.8)
par(mar=c(3,3.5,0,0))
plot(J1$V7[1:200000], type='h', col="#00000080",
     ylim=c(0,120), bty="n", yaxt="n", xaxt="n", cex.axis=0.8,
     ylab="", xlab="")
title(ylab="Read count", line=2)
title(xlab="Position on chr1 (Mb)", line=1.8)
axis(side=1, at=(0:6)*1e5/3, labels=(0:6)*10, cex.axis=0.8, padj=-1)
axis(side=2, at=c(0,40,80,120), cex.axis=0.8)
text(x=-7000, y=115, "JARID1A (rep2)", pos=4, cex=.8)
dev.off()
