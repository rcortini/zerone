...
Here we propose to compare discretizers with an intuitive grpahical
representation (Fig~\ref{fig:expl}). This representation highlights
two key properties discretizers: their conservativity and their
sensitivity. A more conservative discretizer is not better nor worse.
This is a matter of choice because of the inherent trade-off between
false positives and false negatives. However, this choice is usually
implicit and the merit of our representation is to make it explicit.
Equally conservative discretizers may still differ in their false
negative rate or sensitivity. In this case, the more sensitive is
a better option. The second merit of our representation is to show
how far discretizers are from the optimal sensitivity (for their
conservativity).

The Pareto front captures the trade-off between false positive and
false negative rates. Figure \ref{fig:pareto} reveals that this front
has different shapes, depending on the feature being discretized.
This explains the difficulty of finding `univeral' discretization
methods. Zerone stands out as the least conservative of the
discretizers tested here.

Explain that it makes sense.

Explain that the naive approach is not the same.
