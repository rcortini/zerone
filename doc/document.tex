...
CTCF binds a 20~bp consensus sequence that is highly conserved in vertebrates.
In humans, nearly 80\% of the CTCF binding sites contain the consensus motif
\citep{pmid17382889}. In order to determinethe capacity of the different
programs to call peaks of CTCF binding, we compared the discretized profiles
against a data set of CTCF binding motifs. We obtained a reference data set
containing the positions of 85,690 occurences of the CTCF motif
(\textit{c.f.} Section \ref{sub:bench_cond}).

For this comparison, a window identified as enriched will be considered a true
positive if it contains at least one CTCF motif.

The precision of a discretizer is its positive predictive value (\textit{i.e.}
the ratio between true positives and total predicted positives); in our case,
this is the number of windows identified as enriched that contain at least one
CTCF motif, divided by the total number of windows identified as enriched.
Similarly, the recall of a discretizer is its sensitivity (\textit{i.e.} the
ratio between true positives and actual positives); this is the number of true
positives as described above, divided by the total number of windows that
contain at least one CTCF motif, regardless of whether they were identified as
enriched or not.

Both precision and recall are scores comonly used in machine learning contexts.
