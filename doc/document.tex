...
Istead, we used an SVM. SVMs address the classification problem from a
different perspective, they are fast to train, require only one hyperparameter
to be fit, guarantee the lowest overfitting upper bound and, if the training
data is mapped to a kernel space, they can also perform nonlinear
classification, in which case the fit may depend on additional hyperparameters.

In our case, we chose a radial basis function kernel that require to fit one
...
usually observed. In our tests Zerone was the fastest algorithm
(Fig.~\ref{fig:perf}, top row), being a few orders of magnitude faster than
BayesPeak and JAMM, and a few times faster than MACS. This makes Zerone
suitable for usage in large volume pipelines. All programs behaved in a similar
