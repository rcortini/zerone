...
Discretization will be performed regardless of the quality of the
experimental data being analyzed. Features like depth of coverage,
signal-to-noise ratio, technical biases or, specially and most importantly,
dicrepancies among replicates, can have an impact on how acurately enriched
regions are detected. Therefore, a method to separate valid discretizations
from unreliable ones is needed.

We came up with the simple idea of training a classifier with examples of
satisfactory and unsatisfactory discretizations (see section \ref{sub:training}
for details). At first, a linear classifier like logistic regression seemed
useful, but its performance on both training and tests sets was insuficient,
not going below an error rate of 10\% (data not shown) even in representative,
and big enough training sets. Inquiring into the possible reasons for this
limitation we observed that some discretizations of both classes overlaped
in their feature space (Fig.~\ref{fig:pca}) and it was not possible to separate
them by means of a linear classifier.
...
We thought we needed an algorithm capable of producing more complex, nonlinear
hypotheses in order to capture the relationships between features that
differentiated good from bad discretizations.

At first, an artificial neural network seemed appropriate, but they are prone
to overfitting. Choosing the number of hidden layers, the number of neurons
in each layer and the cost functions makes the hyperparameter space
exaggeratedly vast to search it efficiently.

Istead, we used an SVM, an algorithm that addresses the classification problem
from a different perspective: it separates the two classes with the maximum
possible margin, which guarantees the lowest overfitting upper bound. Also,
they are fast to train, require only one hyperparameter to be fit and, if the
training data is mapped to a kernel space, they can also perform nonlinear
classification, in which case the fit may depend on additional hyperparameters.

In our case, we chose a radial basis function kernel that requires to fit one
additional hyperparameter. After the training, our classifier achieved, in this
case, a 95\% of accuracy on test sets.
...
\subsection{Classifier training}\label{sub:training}
After the analysis, Zerone returns the paremeters of the learned model, such as
the estimated transition matrix and the Viterbi path. These features differ
depending on the quality of the discretization and therefore we used them for
training the SVM \citep{Chang2011,e1071}.
