...
background. Fig.~\ref{fig:expr} shows that the expression levels of regions
detected as enriched in H3K36me3 by Zerone are indeed higher than the ones of
non-enriched regions. Zerone detects as not enriched windows with lower
expression than other programs (Fig.~\ref{fig:expr}, left), consistent with
having the highest recall score as discussed above. Together with JAMM, Zerone
is able to detect as enriched windows that are more expressed than the ones
found by the other software (Fig.~\ref{fig:expr}, right). This last observation
is consistent with the fact that JAMM and Zerone have the highest precision
scores when discretizing the CTCF dataset. Overall, the behavior of the tested
software on the H3K36me3 dataset is similar to the observed in the previously
analyzed CTCF dataset.
...
H3K36me3, as determined by the different discretizers. Right: mean fold change
in gene expression of enriched windows relative to non-enriched ones.}
\label{fig:expr}
\end{figure}

% TODO: Talk about overlap between discretizers.
\ref{fig:venn}
