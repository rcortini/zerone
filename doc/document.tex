...
\section{Results:}
  ...
  low quality data with 98\% accuracy. Here we also introduce a
  graphical representation to compare discretizers and we show that
  Zerone achieves outstanding accuracy. Finally, on current hardware,
  Zerone discretizes ChIP-seq experiment on mammalian genomes in about
  5 minutes using less than 800 MB of memory.
...
was wgEncodeSydhTfbsK562InputRawData.fastq.gz.
...
control step taking place after the discretization.
Computer-assisted quality control is essential for high throughput
pipelines and for cases with no prior knowledge. This not only
...
We tested discretizing ENCODE ChIP-seq profiles obtained in human H1
ES cells. Upon discretizing the lysine-demethylase
