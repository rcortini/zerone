...
\section{Results:}
  ...
  low quality data with 98\% accuracy. Zerone is also flexible, fast
...
\caption{
  ...
  8 features extracted from each discretization are projected on the
...
genome assemblies \citep{pmid21059603}.
...
version 1.376 beta (and gem-indexer version 1.423 beta). For convenience,
...
amount of resources. In our benchmark, Zerone used around 40\% more memory
than MACS, but it processed twice as many files. For the same amount
...
% This last sentence is unclear to me (and the proportions changed).
...
We identified 8 summary statistics that characterize the
...
with 98\% accuracy (\textit{c.f.} section \ref{sub:training}).
