  ...
  \section{Results:}
    ...
    low quality profiles with 2\% of errors. We also introduce a graphical
    representation to compare discretization quality and we show that
    Zerone achieves outstanding accuracy. Finally, on current hardware,
...
\caption{
  ...
  over the whole range (colors online).
}\label{fig:ZINB_fit}
\end{figure}
...
not modelled explicitly from known features of the genome (\textit{e.g.}
G+C content), but implicitly by adjusting
...
$x_i$. Discretization amounts to finding the sequence of states and the
...
on the human leukemia cell line K562. The data were
...
represent peaky, mixed and broad signals, respectively---and a
...
For the IDR analysis, we run MACS with a more relaxed qvalue cutoff to obtain
a higher number of peaks. We did this by setting the option \texttt{-q} or
\texttt{--qvalue} to 0.05 (default value 0.01).
The IDR analysis proper was conducted using the R script
\texttt{batch-consistency-analysis.r}, available at
\href{https://sites.google.com/site/anshulkundaje/projects/idr}{https://sites.google.com/site/anshulkundaje/projects/idr}.
Peaks that scored below 0.05 were selected as reproducible.
...
data set on the cell line K562. Its GEO accesion number is GSM1480325. We
considered all the genomic features that overlapped with at least one
GRO-seq tag to be on an expressed region.
...
and the overall $F_1$ score suffers.
...
{\begin{tabular}{lr|rccc|rccc}
        ...
        MACS$^{(1)}$ + IDR &  19,369 & 5,875 & 0.30 & 0.20 & 0.24 & 1,254 & 0.06 & 0.04 & 0.05 \\
...
very neat advantage in precision, only matched by MACS when the complementary
IDR analysis is performed. In this latter case, MACS + IDR achieves similar
scores to those of Zerone, but at the cost of decreasing the total number of
valid peaks found. These results confirm that the performance of Zerone is
outstanding on this data set, as suggested
...
a very neat advantage in precision. In this case, discarding the
nonreproducible peaks found by MACS only gives a mild increase in
precision, that does not reach the performance of Zerone in this data
set. This shows that Zerone accurately describes profiles of this category.
